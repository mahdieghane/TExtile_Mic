...
int featureRowNum = 100;
int featureColumnNum = 100;
...
String[] objects = {"Writing", "Chopping","Tapping", "Clapping" , "Washing hands", "Using toothbrush", "Grating",  "Cleaning", "Drinking", "Chewing","Coughing","Sneezing","Snoring", "Speaking", "Walking", "Standing Up", "Fall detection", "Crunch", "Squat", "Noise"};
int recordingTime = 0;
int[] values;
String result = "";
...
void setup() 
{
  ...
  if(dataStreaming == 0){
    ...
    try{
     ...
     myClient = new Client(this, "localhost", 8084); 
     if(myClient != null) 
       dataStreaming = 1;
  }
  ...
  if(visualizationStyle == 0){
      ...
      for(int i = 0; i < featureColumnNum; i++){
         ...
         if(features[index][i] > 0)
             rect(10+rectWidth*i, 800, rectWidth, -features[index][i]*2);
      }
      ...
      image(spetrogramImage, 0, 50, width, height);
  }
  //if(!isStreaming){
  //  for (int i = 0; i < featureRowNum; i++){
  //       features[index][i] = -1f;
